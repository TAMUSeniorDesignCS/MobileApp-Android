...
public class WritePostActivity extends Activity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		if(getIntent().hasExtra("toEdit")){
			String toEdit = getIntent().getExtras().getString("toEdit");
			postId = getIntent().getExtras().getInt("postIdToEdit");
			EditText edit = (EditText) findViewById(R.id.post_box);
			edit.append(toEdit);
			Button writePostButton = (Button) findViewById(R.id.submit_post);
			writePostButton.setOnClickListener(mEditPostButton);
		}
		else{
	...
	private OnClickListener mEditPostButton = new OnClickListener() {
		...
		public void onClick(View v) {
			...
			if (newPostString.length() == 0) {
				...
				sendPostTask.execute(NetworkAsyncTask.serverLit + "post/edit?postid=" + postId + "&message=" + newPostString + "&timeout=" + timeout + "&rusername=" + username + "&rpassword=" + password);
				try{
